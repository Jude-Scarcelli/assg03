...
uint16_t sign_extend(uint16_t bits, int size)
{
  if ((bits >> (size - 1)) & 1)
  {
    uint16_t mask = 0xFFFF << size;
    return bits | mask;
  }
  else
  {
    uint16_t mask = 0xFFFF >> (16 - size);
    return bits & mask;
  }
}
...
void sti(uint16_t i)
{
  ...
  mem_write(final_address, reg[sr]);
}
...
void br(uint16_t i)
{
  uint16_t cond_flags_in_instruction = DR(i);

  if (cond_flags_in_instruction & reg[RCND])
  {
    reg[RPC] += PCOFF9(i);
  }
}
